# https://github.com/uoe-agents/seac/blob/master/seql/iql.py
...
from a1_marl_utils_orig_seql import soft_update
from c_main_agent_dsattnr_indliql_seql import DSRQAgent
from d_marl_algo_seql import MarlAlgorithm

#device = torch.device("cuda:0" if torch.cuda.is_available() else device)  # Originally
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # By me

MSELoss = torch.nn.MSELoss()  # Originally

# If using cuda  # By me
#MSELoss = torch.nn.MSELoss().to(device)  # Originally
...
class IQL(MarlAlgorithm):
    ...
    def __init__(self, n_agents, observation_sizes, action_sizes, params):
        ...
        """
        super(IQL, self).__init__(n_agents, observation_sizes, action_sizes, params)
        ...
        self.agents = [
            DSRQAgent(observation_sizes[i], action_sizes[i], params)
    ...
    def prep_rollouts(self, device=device):
        ...
        for a in self.agents:
            a.model.eval()
        if device == "gpu":  # Originally == "gpu"; if to use gpu, then device == device
            fn = lambda x: x.cuda()
        else:
            fn = lambda x: x.cpu()
    ...
    def step(self, observations, explore=False, available_actions=None):
        ...
        if available_actions is None:
            ...
            return [a.step(obs, explore, available_actions[i])[0]
    ...
    def prep_training(self, device="gpu"):  # Originally device="gpu"; if to use gpu, then device=device
        """
        ...
        if device == "gpu":   # Originally device == "gpu"; if to use gpu, then device == device
            fn = lambda x: x.cuda()
        else:
            fn = lambda x: x.cpu()
        if not self.model_dev == device:
    ...
    def update_agent(self, sample, agent_i, use_cuda):
        ...
        obs, acs, rews, next_obs, dones = sample # Originally
        ...
        # First back propagation then compute gradients
        qloss.backward()
        torch.nn.utils.clip_grad_norm_(curr_agent.model.parameters(), 0.5)
        # Tell the optimizer the gradients, Gradient Descent, then step to update all parameters (tensors)
    ...
    def update(self, memory, use_cuda=False):
        ...
        q_losses = []
        if use_cuda:
            self.prep_training(device="gpu")   # Originally device="gpu"; if to use gpu, then device=device
        else:
            self.prep_training(device=device)    # Originally device=device
        if self.shared_experience:
        ...
        for a_i in range(self.n_agents):
            ...
            q_loss = self.update_agent(samples, a_i, use_cuda=False)  # Originally "False"; if to use gpu, then use_cuda=True
    ...
    # Originally
    def load_model_networks(self, directory, extension="final"):
        """
        ...
        # By me
        # enumerate(iterable, start=0) ; start param: the index value from which the counter is to be started, by default it is 0
        # Returns an iterator with index and element pairs from the original iterable
        # https://www.geeksforgeeks.org/enumerate-in-python/
        # E.g., It returns the index (i) and element (agent) pairs from the iterable list (self.agents/models).

        for i, agent in enumerate(self.agents):
            #name = "sattn_seql_maher_agent%d_params_" % i  #############################################################
            name += extension
            agent.model.load_state_dict(
                torch.load(os.path.join(directory, name), map_location=device))
            agent.model.eval()  # By me
            #print(f"Agent {i} model {agent.model}")